...
function html5_scale(_bw, _bh, _cw, _ch, _center){
...
var _fscale = floor((_ch / _bh) * 10) / 10;

if ((_cw / _aspect) > _ch)
{
    window_set_size((_bh * _fscale *_aspect), _bh * _fscale);
}
else
{
    window_set_size(_bw * _fscale, ((_bw * _fscale) / _aspect));
}
if (_center)
{
    window_center();
...
show_debug_message( window_get_width() / _bw );

surface_resize(application_surface, min(window_get_width(), _bw), min(window_get_height(), _bh));
display_set_gui_size(640, 480);

window_mouse_set(device_mouse_x_to_gui(0), device_mouse_y_to_gui(0));




//display_set_gui_maximize( window_get_width() / _bw,  window_get_width() / _bw, 0, 0);
